...
res.render('india/inCustBacground',{YOUR_API_KEY: process.env.API_KEY})
})
...
router.post('/tdrv/withdrawal', function(req, res, next) {
    database.pilot.findOne({ pilotID: req.body.pilotID }, function(er, pilot) {
        /////Find Old withdrawal/////////
        database.DriverPayment.find({ pilotID: pilot.pilotID, travelmod: pilot.travelmod, DriverType: "Tride", typeOfReqest: "TrideWidthral" }, function(err, withdrawal) {
            database.DriverPayment({
                pilotID: pilot.pilotID,
                travelmod: pilot.travelmod,
                DriverType: "Tride",
               ...
               // var driverLastCheckAccountBalance = Number(pilot.driverLastCheckAccountBalance) - Number(req.body.withdrawlAmount);
                database.pilot.findOneAndUpdate({ pilotID: pilot.pilotID }, { $set: { lastCheckCashCollcetion: 0, lastCheckFuleconsumption: 0,lastCheckTdriverEarning:0, lastpayoutAmount: 0 } }, function(eerr, dd) {
                    res.render('india/inTdriverWithdrawal', { oldwithdrawal: withdrawal, newReq: req.body, pilot: pilot })
                })
...
router.post('/conframScheduleOrderTride', function(req, res, next) {
    database.ride.findOne({bookingID:req.body.bookingID},function(err,ride){
        if(!ride){
          database.pilot.findOne({pilotID:Number(req.body.pilotID),travelmod:req.body.travelmod},function(err, pilot){
              //console.log("pilot",pilot);
              if(pilot){
                  if(req.body.payMode=="1"){
                      processOrderTride({
                          ...
                          ////Requiest for Preride  List refresh/////
                          res.io.emit("CallAcceptListDisplay", { driverBusy: "busy", pilotID: req.body.pilotID, CustID: req.body.CustID });
                          res.io.emit("startRingtone", { play: "1" });
                          res.io.emit("bookingFromSubAdmin", { pilotID: req.body.pilotID , CustID: req.body.CustID});
                          res.send('OK');    
                      });
  ...
  function processOrderTride(req,res){
    database.schedule.findOne({bookingID:req.bookingID},function(err,schedule){
      if(schedule){
        database.ride({

        bookingID:schedule.bookingID,
        travelmod:schedule.travelmod,
        CustID:schedule.CustID,
        pilotID:req.pilotID,
        DriverType: "Tride",
        picupaddress: schedule.originAds,
...
router.get('/inPreDriverBackGroundLocation', function(req, res, next) {
    if (req.cookies.pilotID) {
        res.render('india/inPreDriverBackGroundLocation', { YOUR_API_KEY: process.env.API_KEY })
    }
});
                ...
                database.Carlogbook.findOne({ bookingID: req.body.bookingID }, function(err, carlog) {
                    ...
                    if(carlog){
                        if(carlog.pickupDistance){
                            ...
                            if(Number(carlog.pickupDistance) > 3 && Number(carlog.pickupDistance) <= 6){
                                longPickup=5; 
                            }else{
                                if(Number(carlog.pickupDistance) > 6){
                                    longPickup=10; 
                                }
                            }
                ...
                if(key===ary.length - 1){
                    ...
                    database.pilotIncentiveModule.find({incetiveCategry:req.incetiveCategry,travelmod: req.travelmod,city:req.city},function(er, incModul){
                        ...
                        if(incModul.length >0 ){
                            var slot=Number(incetSlot.length)+1;
                            let obj = incModul.find(o => o.incetiveSlot === slot.toString());
                            if(obj){
                                if(Number(obj.inctravelKM) <= Number(kmTravels) + Number(req.thisOrderkm) && Number(obj.numberOfBooking) <= Number(orderCount.length)+1){
                                    cb(Number(obj.incetiveAmount)); 
                                }else{
                                 cb(0);   
                                }
                            }else{
                        ...
                        //        // console.log("value.inctravelKM",value.inctravelKM,"kmTravels",kmTravels,"value.incetiveSlot",value.incetiveSlot,"incetSlot.length",incetSlot.length,"value.numberOfBooking",value.numberOfBooking,"orderCount.length",orderCount.length)
                        //      if(Number(value.incetiveSlot) == Number(incetSlot.length)+1){
                        //          if(Number(value.inctravelKM) <= Number(kmTravels) + Number(req.thisOrderkm) && Number(value.numberOfBooking) <= Number(orderCount.length)+1){
                        //             incentv=value.incetiveAmount;  
                        //             incentiveCount=1;
                        //            // console.log("incentv",incentv)
                        //          }else{
                        //              if(incentiveCount==0){
                        //                 incentv=0;
                        //              }
            ...
            database.ride.find({
                ...
                if (balance.length > 0) {
                    balance.forEach(function(val, key, ary) {
                        ...
                        if (val.longPickup) {
                            longPickupAmt = Number(longPickupAmt) + Number(val.longPickup);
                        ...
                        if (key === ary.length - 1) {
                            var priviousEarning = Number(driverpayout) + Number(incentive) +  Number(longPickupAmt) + Number(fuelCost) + Number(maintenance);
                            var previousDue = Number(cashCollection);
    ...
    database.ride.find({
        ...
        if (balance.length > 0) {
            balance.forEach(function(val, key, ary) {
                ...
                if (key === ary.length - 1) {
                    cb({ cashCollection: cashCollection, driverpayout: driverpayout, incentive: incentive, maintenance: maintenance, fuelCost: fuelCost, noOfBooking: ary.length, noOflongPickup:noOflongPickup, longPickupAmt:longPickupAmt,kmTravels:kmTravels});
                }
            });
        } else {
            cb({ cashCollection: 0, driverpayout: 0, incentive: 0, maintenance: 0, fuelCost: 0, noOfBooking: 0, noOflongPickup:0, longPickupAmt:0, kmTravels:0});
        }
    ...
    if (response.resdata) {
        database.paygetway({
            ...
            if (response.status == "captured") {
                ...
                }else if (response.resdata.payment_token.udf.PaymentType == "TrideDeposit"){
                    database.pilot.findOneAndUpdate({ pilotID: response.resdata.payment_token.udf.CustID }, {
                        $set: {
                            lastCheckCashCollcetion: 0,
                            lastCheckTdriverEarning:0,
                            lastCheckFuleconsumption: 0,
                            lastpayoutAmount: 0,
...
router.get('/admin/gstReport', function(req, res, next) {
    if (req.cookies.adminID) {
        res.render('admin/india/gstReport.ejs');
...
router.post('/admin/sub/dropAlldriver', function(req, res, next) {
    database.driverlocation.deleteMany({DriverType:"Tride"}, function(e, ddd) {
        if (ddd) {
            res.send("success");
        } else {
            res.send("fail");
...
router.post('/admin/sub/tDrvIsAnyBooking', function(req, res, next) {
    database.ride.find({ pilotID: req.body.pilotID, driverBusy: "busy", DriverType: "Tride" }, function(err, data) {
        res.send({ order: data });
    })
    
})

function geoPlaceSearch(req,cb){
    database.index2DplaceSearch({}, function(ss) {
        /////Check  Driver for 3KM //////
    var lng=22.9999;
    var lat=88.666;
        
        database.geocode.find({
            location: {
                $near: {
                    $geometry: {
                        type: "Point",
...
// geoPlaceSearch({lat:23.5204, lng:87.3119},function(res){
//   console.log(res);
// });
...
router.post('/admin/incentiveModule', function(req, res, next) {
    database.pilotIncentiveModule.find({city:req.body.incetiveCity,travelmod:req.body.incentiveTravelMode,incetiveCategry:req.body.incentiveCategory},function(er,data){
        res.send(data);
    })
 });
...
router.post('/admin/sub/getTrideID', function(req, res, next) {
    database.pilot.findOne({$or:[ {pilotID: req.body.mobile}, {mobileNumber: req.body.mobile, isdCode: req.body.isdCode}] }, function(err, data) {
        if (data) {
            res.send(''+data.pilotID+'');
        } else {
            res.send('worng');
        }
...
router.post('/conframDelivery', function(req, res, next) {
    GenbookingID({}, function(NewBookinid) {
        ...
        database.delivery({
            ...
            compensetion:req.body.compensetion
...
router.post('/ringWeakupRequiest', function(req, res, next) {
 ...
 router.post('/walletRechargeApi', function(req, res, next) {
    
 
});

 router.post('/getPriceApi', function(req, res, next) {
    
 
});

 router.post('/createOrderApi', function(req, res, next) {
    
 
 });

 router.post('/orderDetailsApi', function(req, res, next) {
...
//console.log(new Date().getHours());
        ...
        // var request = require('request');
        // var url = "https://paacab.com/uk/orderlist";
        // var options = {
        //     method: 'POST',
        //     url: url,
        //     headers: {
        ...
        //         pickupName: 'Pizza King',
        //         pickupAddress: 'Brixton, London, UK',
        //         pickupLocation: [ -0.1156148, 51.4612794 ],
        //         pickupContact: '98568694755',
        //         dropName: 'xyz',
        //         dropAddress: 'hdhs',
        //         dropHome_no: 'Shaban',
        //         dropLocation: [ -0.1156148, 51.4612794 ],
        //         dropContact: '44756135891'
        //     },
        //     json: true
        // };
        ...
        // var options = {
        // 'method': 'POST',
        // 'url': 'https://paacab.com/uk/orderlist',
        // 'headers': {
...
router.post('/testIncentive', function(req, res, next) {
    database.pilot.findOne({ pilotID: req.body.pilotID }, function(re, driver) {
        incentiveFn({
            pilotID: driver.pilotID,
            travelmod: driver.travelmod,
            driverCetegory:driver.driverCetegory,
            incetiveCategry:driver.incentiveMode,
            city:driver.cityName,
            thisOrderkm:req.body.kmtravels
        }, function(insentive){
           res.send({insentive:insentive}) 
        })
    })
        ...
        // var request = require('request');
        // var url = "https://paacab.com/india/testIncentive";
        // var options = {
        //     method: 'POST',
        //     url: url,
        //     headers: {
...
//console.log(JSON.parse('{\"pickupName\":\"Burger King\",\"pickupAddress\":\"3 Blenheim St, Mayfair, London W1S 1LA, UK\",\"pickupLocation\":[-0.1337,51.5081548283056],\"pickupContact\":\"452136987\",\"dropName\":\"mansi\",\"dropAddress\":\"108 Shaftesbury Ave\",\"dropHome_no\":\"A-420\",\"dropLocation\":[0,0],\"dropContact\":\"7894561285\"}'))
    


        // var request = require('request');
        // var url = "http://52.5.151.226:3000/api/v2/admin/OrderInfo";
        // var options = {
        //     method: 'GET',
        //     url: url,
        //     headers: {
        //         'content-type': 'application/json',
        //         _id:'600a5d34c343ed3026227c4e',
        //     },
        //     body: {
        //         _id:'600a5d34c343ed3026227c4e',
        ...
        // var request = require('request');
        // var url = "http://52.5.151.226:3000/api/v2/admin/OrderStatusUpdate";
        // var options = {
        //     method: 'PUT',
        //     url: url,
        //     headers: {
