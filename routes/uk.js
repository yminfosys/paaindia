            ...
            if (data) {
                if (data.status == "Active") {
                    ...
                    if (data.orderStage == 'accept' || data.orderStage == 'startRide' || data.orderStage == 'finishRide') {
                        ...
                        res.render('uk/inCust', { YOUR_API_KEY: process.env.API_KEY, error: '', cust: data, geocode: geocode, wallet: wallet })


                    }

                } else {
                    res.render('uk/inCust', { YOUR_API_KEY: process.env.API_KEY, error: 'Your Account is De-Active Contact Customer Care to Active your Account',wallet: wallet })
                }

            } else {
                res.render('uk/inCust', { YOUR_API_KEY: process.env.API_KEY, error: 'cookes' ,wallet: wallet})
            }
        });
...
router.get('/custBackground', function(req, res, next) {
res.render('uk/inCustBacground',{YOUR_API_KEY: process.env.API_KEY})
})
...
router.post('/login', function(req, res, next) {
    database.customer.findOne({ mobileNumber: req.body.mobile, isdCode: '+44' }, function(err, user) {
        if (user) {
...
router.post('/setnewpassword', function(req, res, next) {
    bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
        database.customer.findOneAndUpdate({ mobileNumber: req.body.mobile, isdCode: '+44' }, { $set: { password: hash } }, function(err, user) {
            if (user) {
                res.send("Success");
            }
        });
...
router.get('/ride', function(req, res, next) {
    if (req.cookies.CustID) {
        database.customer.findOne({ CustID: req.cookies.CustID }, function(err, cust) {
            if (cust.bookingID) {
                res.render('uk/inCustRideConfrm', { YOUR_API_KEY: process.env.API_KEY, orderStage: cust.orderStage, bookingID: cust.bookingID, cust: cust })
            } else {
                res.redirect('/uk')
            }
        ...
        database.pilot.findOne({ completereg: 'done', pilotID: req.cookies.pilotID }, function(err, data) {
            ...
            if (data) {
                ...
                database.pilot.findOne({ pilotID: req.cookies.pilotID }, function(err, driver) {
                    ...
                    res.render('uk/inDriverReg', { YOUR_API_KEY: process.env.API_KEY, driver: driver, submitvar: dd });
                });
...
router.post('/drv/completeReg', function(req, res, next) {
    ...
    res.redirect('/uk/drv?reg=submit')
...
/////////Check Document Expeairy/////////////////
router.post('/checkDocExpairy', function(req, res, next) {
  database.pilot.findOne({pilotID:req.body.pilotID},function(err,data){
      if(data){
          if(data.dlExpDate && data.insExpDate){
            var dlExpDate=new Date(data.dlExpDate).getTime();
            var insExpDate= new Date(data.insExpDate).getTime();
            var today=new Date().getTime();
            console.log(dlExpDate,insExpDate)
            if(today>=dlExpDate || today>=insExpDate ){
                database.pilot.findOneAndUpdate({ pilotID:req.body.pilotID }, { $set: { completereg: '', accountStatus: '' } }, function(err, data) {
                    res.send("Deactive"); 
                })
            }else{
                res.send("Active"); 
...
////PreRide Shedule trip//////
router.post('/setpickuptimesetup', function(req, res, next) {
    ...
    GenbookingID({}, function(NewBookinid) {
        database.schedule({
            ...
            perMinuteTimeCost:req.body.perMinuteTimeCost,
            totalDistance:req.body.totalDistance,
            travalTime:req.body.travalTime,
            payMode:req.body.payMode,
            pickupTime:req.body.pickupTime,
            pickupDate:req.body.pickupDate,   ////moment(""+req.body.pickupDate+" 09:00:00").toISOString(), ////req.body.pickupDate,/////moment(req.body.pickupDate).utc(),   ///new Date(''+req.body.pickupDate+'T'+req.body.pickupTime+''), /////new Date(),////req.body.pickupDate,   //////new Date(''+req.body.pickupDate+'T'+req.body.pickupTime+'').utc(),
            callbookingStatus:req.body.callbookingStatus,
            preRideOTP: otp
        }).save(function(err) {
            res.send({bookingID:NewBookinid.bookingID,otp:otp})
        })
    })
});

router.post('/getScheduleJourney', function(req, res, next) {

    database.schedule.find({CustID:req.body.CustID, callbookingStatus: "InitByCust"},function(err, data){
        console.log(data)
        res.send(data);
...
router.post('/deleteSchdule', function(req, res, next) {

    database.schedule.findOneAndUpdate({bookingID:req.body.bookingID},{$set:{
        callbookingStatus: "CancelByCustomer" 
    }},function(err, data){
        if(data){
            res.send("OK")
        }else{
...
router.post('/getScheduleOrder', function(req, res, next) {
    var StartTime = moment(req.body.serchDate).startOf('day').utc();
    var EndTime = moment(req.body.serchDate).endOf('day').utc();
    database.schedule.find({
        pickupDate: { $gte: StartTime.toDate(), $lte: EndTime.toDate() },
...
router.post('/getCustDetails', function(req, res, next) {
    database.customer.findOne({CustID:req.body.CustID},function(err, cust){
        res.send(cust);
    })
...
router.post('/conframScheduleOrder', function(req, res, next) {
  database.ride.findOne({bookingID:req.body.bookingID},function(err,ride){
      if(!ride){
        database.pilot.findOne({pilotID:Number(req.body.pilotID),travelmod:req.body.travelmod},function(err, pilot){
            console.log("pilot",pilot);
            if(pilot){
                if(req.body.payMode=="1"){
                    processOrder({
                        pilotID:req.body.pilotID,
                        bookingID:req.body.bookingID 
                    },function(d){
                        ...
                        res.io.emit("bookingFromSubAdmin", { pilotID: req.body.pilotID , CustID: req.body.CustID});
                        res.send('OK');    
                    });
                }else if(req.body.payMode=="2"){
                    //////Check Wallet//////
                    database.customer.findOne({CustID:req.body.CustID},function(err, cust){
                        if(cust){
                            if(Number(cust.walletBalance) > Number(req.body.Amt)){
                                processOrder({
                                    pilotID:req.body.pilotID,
                                    bookingID:req.body.bookingID 
                                },function(d){
                                ...
                                res.send('Not Enough Balance In Customer Account');  
                            }
                        }
                    })
                    
                }else if(req.body.payMode=="3"){
                     //////Check Buy KM/ Free //////
                     processOrder({
                        pilotID:req.body.pilotID,
                        bookingID:req.body.bookingID 
...
function processOrder(req,res){
    database.schedule.findOne({bookingID:req.bookingID},function(err,schedule){
      if(schedule){
        database.ride({

        bookingID:schedule.bookingID,
        CustID:schedule.CustID,
        pilotID:req.pilotID,
        DriverType: schedule.DriverType,
        picupaddress: schedule.originAds,
        picuklatlng:[schedule.originLat, schedule.originLng],
        dropaddress:schedule.distAds,
        droplatlng:[schedule.distLat, schedule.distLng],
        kmtravels: schedule.totalDistance,
        totalamount: schedule.totalAmt,
        timefare: schedule.timeFare,
        perMinuteCost: schedule.perMinuteTimeCost,
        paymentBy: schedule.payMode,
        travalTime: Number(schedule.travalTime)+2,
        callbookingStatus: "Accept",
        driverBusy: "busy",
        preRideOTP:schedule.preRideOTP
        }).save(function(err){
            //////CUST data////
        database.customer.findOneAndUpdate({ CustID: schedule.CustID}, {
            $set: {
                orderStage: 'accept',
                bookingID: schedule.bookingID
            }
        }, function(er, cust) {
            database.pilot.findOne({ pilotID: req.pilotID }, function(er, pilot) {
                var newtotalTime = pilot.preRideTotalTime + Number(schedule.travalTime)+2
                database.pilot.findOneAndUpdate({ pilotID: req.pilotID }, {
                    $set: {
                        preRideTotalTime: newtotalTime
                    }
                }, function(er, pil) {
                    database.schedule.findOneAndUpdate({bookingID:req.bookingID},{$set:{
                        callbookingStatus: "ConfirmByAdmin"
                    }},function(er,data){
                        res("ok");
                    })
                })
...
router.post('/cancelScheduleOrder', function(req, res, next) {
    database.schedule.findOneAndUpdate({bookingID:req.body.bookingID},{$set:{
        callbookingStatus: "cancelByAdmin"
    }},function(er,data){
        res.send("OK")
    })
});
...
router.post('/checkAnyOrderExist', function(req, res, next) {
    database.customer.findOne({CustID:req.body.CustID,orderStage:"accept"},function(err,data){
        if(data){
            res.send("HAS_ORDER");
        }else{
            res.send("NOT_ORDER");
...
router.get('/inPreDriverBackGroundLocation', function(req, res, next) {
    if (req.cookies.pilotID) {
        res.render('uk/inPreDriverBackGroundLocation', { YOUR_API_KEY: process.env.API_KEY })
    }
});
...
router.post('/AndroidDriverLocationUpdate', function(req, res, next) {
    console.log(req.body);
       
        //res.io.emit("testgps",{lat:req.body.lat, lng:req.body.lng, accuracy:req.body.accuracy});
        database.pilot.findOne({ pilotID: req.body.pilotID }, function(err, pilot) {
            if (pilot) {
                database.driverlocation.findOne({ pilotID: req.body.pilotID }, function(err, data) {
                    if (data) {
                        database.driverlocation.findOneAndUpdate({ pilotID: req.body.pilotID }, {
                            $set: {
                                pilotID: req.body.pilotID,
                                DriverType: req.body.DriverType,
                                rating: pilot.rating,
...
console.log(req.body);
console.log('req.cookies.pilotID',req.cookies.pilotID)
    var lat=Number(req.body.lat);
    var lng=Number(req.body.lng);
    var accuracy=Number(req.body.accuracy);
    res.cookie("position", JSON.stringify({ lat: lat, lng: lng, accuracy: accuracy }), { maxAge: 30 * 24 * 60 * 60 * 1000 });
...
router.get('/tdrv', function(req, res, next) {
    ...
    if (req.cookies.pilotID) {
        database.pilot.findOne({ completereg: 'done', pilotID: req.cookies.pilotID }, function(err, data) {
            console.log(req.cookies.pilotID)
            if (data) {
                res.render('uk/inTdriver', { YOUR_API_KEY: process.env.API_KEY, driver: data });
            } else {
                database.pilot.findOne({ pilotID: req.cookies.pilotID }, function(err, driver) {
                    if (req.query.reg) {
                        dd = req.query.reg;
                    }
                    res.render('uk/inTdriverRegContinue', { driver: driver, submitvar: dd });
                });
...
router.get('/tdrv/login', function(req, res, next) {
    ...
    if (req.cookies.pilotID) {
        res.redirect('/uk/tdrv')
    } else {
        res.render('uk/inTdriverLogin', { msg: "", reg: d });
    }
})
        ...
        if (data) {
            bcrypt.compare(req.body.password, data.password, function(err, pass) {
                ...
                if (pass) {
                    ...
                    //////Worng Password//////
                    res.render('uk/inTdriverLogin', { msg: "Worng Password Not Match ", reg: "" });
                }
            });
        } else {
            res.render('uk/inTdriverLogin', { msg: "Mobile Number not Exist Register Now", reg: "" });
        }
    });
...
router.get('/tdrv/reg', function(req, res, next) {
    res.render('uk/inTdriverReg', { msg: "", reg: "1" });
})

router.get('/tdrv/reg/otp', function(req, res, next) {
    res.redirect('/uk/tdrv/reg')
})

router.post('/tdrv/reg/otp', function(req, res, next) {
    var otp = randamNumber();
    database.pilot.findOne({ mobileNumber: req.body.mobile, isdCode: '+44' }, function(err, pilot) {
        if (!pilot) {
            // console.log("OTP", otp)
            // res.render('uk/inTdriverReg',{msg:"",reg:"2", otp:otp, mobile:req.body.mobile});
            smsotp.otpsend({
                apikey: process.env.OTP2KEY,
                mobileno: "+44" + req.body.mobile + "",
                otp: "" + otp + ""
                ...
                if (data.Status == 'Success') {
                    ...
                    res.render('uk/inTdriverReg', { msg: data.Details, reg: "1" });
                }
            })
        } else {
            res.render('uk/inTdriverReg', { msg: "Driver Alredy Register With Us ", reg: "1" });
        }
    })
})
...
router.post('/tdrv/reg/otpverify', function(req, res, next) {
    if (req.body.otp == req.body.otp1) {
        res.render('uk/inTdriverReg', { msg: "", reg: "3", mobile: req.body.mobile, latlng: JSON.parse(req.cookies.reglatlng) });
    } else {
        res.render('uk/inTdriverReg', { msg: "OTP Mismactch", reg: "2", otp: req.body.otp, mobile: req.body.mobile });
    }
...
router.post('/tdrv/driverReg', function(req, res, next) {
    bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
        database.pilot({
            ...
            res.redirect('/uk/tdrv/login?reg=regSuccess');
        });
    });
...
router.post('/tdrv/completeReg', cpUpload, async function(req, res, next) {
    ...
    if (req.files.file1) {
       // var urlphoto = 'driverDocument/photo' + req.body.mobile + '1' + req.files.file1[0].originalname + '';
        ...
        //var urlid='public/uk/driverDocument/id'+req.body.mobile+'1'+id.name+'';
        ...
        await database.pilot.findOneAndUpdate({ mobileNumber: req.body.mobile, isdCode: req.body.isd }, {
            ...
            //console.log(req.body)
            //res.redirect('/uk/tdrv?reg=submit')
...
router.get('/tdrv/forget', function(req, res, next) {
    res.render('uk/inTdriverForgetpsw', { msg: "", fgp: "1" });
})
...
router.post('/tdrv/forget', function(req, res, next) {
    ...
    database.pilot.findOne({ mobileNumber: req.body.mobile, isdCode: '+44' }, function(err, pilot) {
        if (pilot) {
            // console.log("OTP", otp)
            // res.render('uk/inTdriverForgetpsw',{msg:"",fgp:"2", otp:otp, mobile:req.body.mobile});
            smsotp.otpsend({
                apikey: process.env.OTP2KEY,
                mobileno: "+44" + req.body.mobile + "",
                otp: "" + otp + ""
                ...
                if (data.Status == 'Success') {
                    ...
                    res.render('uk/inTdriverForgetpsw', { msg: data.Details, fgp: "1" });
                }
            })
        } else {
            res.render('uk/inTdriverForgetpsw', { msg: "Mobile Number Not Register With us", fgp: "1" });
        }
    })

    //res.redirect('/uk/tdrv/login?psw=Success');
})

router.post('/tdrv/forgetveryfyotp', function(req, res, next) {
    if (req.body.otp == req.body.otp1) {
        res.render('uk/inTdriverForgetpsw', { msg: "", fgp: "3", mobile: req.body.mobile });
    } else {
        res.render('uk/inTdriverForgetpsw', { msg: "OTP Mismactch", fgp: "2", otp: req.body.otp, mobile: req.body.mobile });
    }
...
router.post('/tdrv/resetPassword', function(req, res, next) {
    if (req.body.password == req.body.password1) {
        bcrypt.hash(req.body.password, saltRounds, function(err, hash) {
            database.pilot.findOneAndUpdate({ mobileNumber: req.body.mobile, isdCode: '+44' }, { $set: { password: hash } }, function(err, user) {
                if (user) {
                    res.redirect('/uk/tdrv/login?reg=resetPassword');
                }
            });
        })
    } else {
        res.render('uk/inTdriverForgetpsw', { msg: "Password Not Match", fgp: "3", mobile: req.body.mobile });
    }
...
router.post('/tdrv/startRide', function(req, res, next) {
    database.customer.findOneAndUpdate({ CustID: req.body.CustID }, { $set: { orderStage: 'startRide' } }, function(er, cust) {
        database.pilot.findOneAndUpdate({ pilotID: req.cookies.pilotID }, { $set: { orderStage: 'startRide', mapclick: "" } }, function(re, pilot) {
            database.ride.findOneAndUpdate({ bookingID: req.body.bookingID }, { $set: { startTime: new Date() } }, function(re, ride) {
                res.io.emit("StartRide", { CustID: req.body.CustID });
                res.send({ cust: cust, ride: ride });
            });
            ...
            database.ride.findOne({ bookingID: req.body.bookingID }, function(re, booking) {
                database.Carlogbook.findOne({ bookingID: req.body.bookingID }, function(err, carlog) {
                    ...
                    if(carlog){
                        if(carlog.pickupDistance){
                            ...
                            ////////// Calculate For long Picup////////
                            if(Number(carlog.pickupDistance) > 3){
                                longPickup=10; 
                            }

                        
                        }else{
                            var fuelDistance =  Number(booking.kmtravels);
                        }
                    }
                ...
                if(Number(driver.driverCetegory)== 1){
                    ...
                    incentiveFn({
                        ...
                        //console.log("insentive",insentive)
                        ...
                        tdriverPayout({
                         ...
                         distance:booking.kmtravels
                            ...
                            //console.log("driverpayout",driverpayout)
                        ////////Bill Amount/////
                        var spacelDiscount = 0;
                        if (cust.spacelDiscount) {
                            spacelDiscount = Number(cust.spacelDiscount);
                            ...
                            database.pilot.findOneAndUpdate({ pilotID: req.cookies.pilotID }, { $set: { orderStage: 'finishRide' } }, function(re, driver) {
                                database.ride.findOneAndUpdate({ bookingID: req.body.bookingID }, {
                                    $set: {
                                        ...
                                        totalTime: totalTime,
                                        driverIncentiv: Number(insentive),
...
function incentiveFn(req,cb){
    ...
    database.ride.find({
        ...
        console.log("incetSlot",incetSlot)
        database.ride.find({
            date: { $gte: StartTime.toDate(), $lte: EndTime.toDate() },
            pilotID: req.pilotID,
            travelmod: req.travelmod,
...
router.post('/finishAndFeedback', function(req, res, next) {
    res.io.emit("finishAndFeedback", { CustID: req.body.CustID });
    res.send("finishAndFeedback");
})
...
router.get('/tdriverBackground', function(req, res, next) {
    if (req.cookies.pilotID) {
        res.render('uk/inTdriverBackground', { YOUR_API_KEY: process.env.API_KEY })
    }
});
...
//   INDUSTRY : 'Retail',  
//   WEBSITE : 'DEFAULT',
//   CALLBACK_URL : 'http://localhost:8080/uk/paytm'  //'https://paacab.com/uk/paytm',  //  'http://localhost:8080/uk/paytm' ///// webhook url for verifying payment
// }
...
var return_url = "https://paacab.com/uk/icici";
//var return_url="http://localhost:8080/uk/icici";
    ...
    request(options, function(error, response, body) {
        ...
        savegetwaydata({ status: req.body.status, resdata: body }, function(data) {
            ...
            if (data.status == "captured") {
                res.redirect('/uk/openSucccess?payment=' + data.res.amount + '&od=' + data.res.payment_token.mtx + '&PaymentType=' + data.res.payment_token.udf.PaymentType + '')
            } else {
                if (data.res.payment_token.mtx) {
                    res.redirect('/uk/openUnsucccess?payment=' + data.res.amount + '&od=' + data.res.payment_token.mtx + '&status=' + data.res.status + '')
                } else {
                    res.redirect('/uk/openUnsucccess?od=""')
                }
...
router.get('/openSucccess', function(req, res, next) {
    ...
    // /res.send(req.query);
    res.render('uk/openSuccess', { payGetway: req.query })

});

router.get('/openUnsucccess', function(req, res, next) {
    //res.send(req.query);
    res.render('uk/openUnsucccess', { payGetway: req.query })
});
...
router.post('/admin/getCusbalacce', function(req, res, next) {
    database.customer.findOne({mobileNumber:req.body.mobile},function(err,cust){
        if(cust){
            res.send({wellet:cust.walletBalance,name:cust.name,mobileNumber:cust.mobileNumber});
        }else{
            res.send("Error");
        }
    })
...
router.post('/admin/addWalletMony', function(req, res, next) {
    var newWallet=Number(req.body.walletBalance) + Number(req.body.money)
    database.customer.findOneAndUpdate({mobileNumber:req.body.mobileNumber},{$set:{
        walletBalance:newWallet
    }},function(er,data){
        res.send("New Balance: "+newWallet+"")
    });

});


router.post('/admin/orderAmountModify', function(req, res, next) {
   database.ride.findOne({bookingID:req.body.bookingID},function(err,ride){
    if(ride){
        res.send(ride);
    }else{
...
router.post('/admin/orderAmountUpdate', function(req, res, next) {
    var newWallet=Number(req.body.walletBalance) + Number(req.body.money)
    var driverCashCollection=0;
    var totalamount=Number(req.body.prndingAmount) + Number(req.body.spacelDiscount);
    if(req.body.paymentBy=="1"){
        driverCashCollection=Number(req.body.timefare)+Number(req.body.prndingAmount);
    }
   
    database.ride.findOneAndUpdate({bookingID:req.body.bookingID},{$set:{
        totalamount: totalamount,
        timefare: req.body.timefare,
        driverCashCollectio: driverCashCollection
    }},function(er,data){
        res.send("New driverCashCollection: "+driverCashCollection+"")
    });
...
router.get('/admin/ridedetails', function(req, res, next) {
        res.render('admin/uk/rideDetails', { title: 'Paacab' });
    })
    // var mont='12';
...
router.get('/admin/sub', function(req, res, next) {
    if (req.cookies.subAdminID) {
        ...
        database.admin.findOne({ adminID: req.cookies.subAdminID }, function(err, admin) {
            if (admin) {
                bcrypt.compare("admin", admin.password, function(err, pass) {
                    if (pass) {
                        /////set New Password/////
                        res.render('admin/uk/subAdminNewPsw', { admin: admin });
                                ...
                                res.render('admin/uk/appAdminSub', { title: 'Paacab', driverMobile:driverMobile });
        ...
        res.redirect('/uk/admin/sub/login');
    }
...
router.get('/admin/sub/login', function(req, res, next) {
    if (req.cookies.subAdminID) {
        res.redirect('../../uk/admin/sub');
    } else {
        res.render('admin/uk/subAdminLogin', { msg: "" });
    }
        ...
        if (user) {
            bcrypt.compare(req.body.password, user.password, function(err, pass) {
                ...
                if (pass) {
                    ...
                    //////Worng Password//////
                    res.render('admin/uk/subAdminLogin', { msg: "Worng Password" });
                }
            });
        } else {
            ////////Register Admin///////

            res.render('admin/uk/subAdminLogin', { msg: "Worng User Email" });
...
router.post('/admin/sub/uploadfile', cpUpload, async function(req, res, next) {
    ...
    if (req.files.file1) {
        // var urlphoto = 'driverDocument/photo' + req.body.mobile + '1' + req.files.file1[0].originalname + '';
         ...
         //var urlid='public/uk/driverDocument/id'+req.body.mobile+'1'+id.name+'';
         // await imgresizer(req.files.file1[0].buffer, url, 720, 720)
         await database.pilot.findOneAndUpdate({ mobileNumber: req.body.driverMobile, isdCode: "+44"}, {
             $set: {
                 photo: url
             }
         }, function(e, d) {
            res.redirect('/uk/admin/sub?m='+req.body.driverMobile+'');
         });
     }
...
////////////////////////Finish Ride By Sub Admin/////////////////////////

router.post('/admin/sub/preRideFinishBySubadmin', function(req, res, next) {
    database.ride.findOne({ bookingID: req.body.bookingID}, function(re, booking) {
      database.pilot.findOne({ pilotID: booking.pilotID }, function(re, driver) {
        database.customer.findOne({ CustID: booking.CustID }, function(re, cust) {
                database.Carlogbook.findOne({ bookingID: booking.bookingID }, function(err, carlog) {
                    ///////Total Travel TimeFare//////
                    var endTime = new Date();
                    var travelmod = driver.travelmod;
                    var totalTime = endTime.getTime() - moment(booking.startTime).utc().toDate().getTime();
                    ...
                    ////////Save and Update Finish////////
                    database.customer.findOneAndUpdate({ CustID: booking.CustID }, { $set: { orderStage: 'finishRide' } }, function(er, cust) {
                        database.pilot.findOneAndUpdate({ pilotID: booking.pilotID }, { $set: { orderStage: 'finishRide' } }, function(re, driver) {
                            database.ride.findOneAndUpdate({ bookingID: booking.bookingID }, {
                                $set: {
                                    driverFuelConsumtion: fuleConsumption,
                                    totalTime: totalTime,
                                    spacelDiscount: spacelDiscount,
                                ...
                                if (updatbooking) {
                                    ...
                                    if (Number(updatbooking.paymentBy) == 2) {
                                        var walletAmt = Number(cust.walletBalance) - billAmount;
                                        database.customer.findOneAndUpdate({ CustID: booking.CustID }, { $set: { walletBalance: walletAmt } }, function(er, cu) {
                                            res.io.emit("finishRide", { CustID: booking.CustID });
                                            res.send({ billAmount: 0 });
                                        });
                                    } else {
                                        if (Number(updatbooking.paymentBy) == 3) {
                                            var buykmAmt = Number(cust.BuyKM) - Number(booking.kmtravels);
                                            database.customer.findOneAndUpdate({ CustID: booking.CustID }, { $set: { BuyKM: buykmAmt } }, function(er, n) {
                                                res.io.emit("finishRide", { CustID: booking.CustID });
                                                res.send({ billAmount: 0 });
                                            });
                                        } else {
                                            database.ride.findOneAndUpdate({ bookingID: booking.bookingID }, { $set: { driverCashCollectio: billAmount } }, function(er, cash) {
                                                res.io.emit("finishRide", { CustID: booking.CustID });
                                                res.send({ billAmount: billAmount });
                                            });
                                        }
...
router.post('/chatisConnecting', function(req, res, next) {
    database.customer.findOne({CustID:req.body.CustID},function(err,cust){
        if(cust){
            res.io.emit("chatisConnecting",{mobileNumber:cust.mobileNumber, CustName:cust.name, CityName:cust.CityName,CustID:cust.CustID})
            res.send('chatisConnecting emit');
        }
    })
...
router.post('/chatisConnectingSuccess', function(req, res, next) {
    res.io.emit("chatisConnectingSuccess",{CustID:req.body.CustID})
    res.send('chatisConnecting emit');
})


router.post('/chatisConnectingAccept', function(req, res, next) {
    var ACCEPT_SESSION=new Date().getTime();
    res.io.emit("chatisConnectingAccept",{CustID:req.body.CustID,ACCEPT_SESSION:ACCEPT_SESSION})
    res.send({ACCEPT_SESSION:ACCEPT_SESSION});
})

router.post('/chatisConnectingReject', function(req, res, next) {
    res.io.emit("chatReject",{CustID:req.body.CustID})
    res.send('chatisConnecting emit');
})

router.post('/sendChatContent', function(req, res, next) {
    res.io.emit("receiveChetContent",{CustID:req.body.CustID,chatSession:req.body.chatSession,chatContent:req.body.chatContent})
    res.send('receiveChetContent');
})

router.post('/sendChatContentAdmin', function(req, res, next) {
    res.io.emit("receiveChetContentAdmin",{CustID:req.body.CustID,chatSession:req.body.chatSession,chatContent:req.body.chatContent})
    res.send('receiveChetContent');
})



router.post('/chatSessionDisconnect', function(req, res, next) {
    res.io.emit("chatSessionDisconnect",{sessionID:req.body.sessionID});
    res.send('chatSessionDisconnect');
})
...
router.get('/admin/gstReport', function(req, res, next) {
    if (req.cookies.adminID) {
        res.render('admin/uk/gstReport.ejs');
...
router.post('/admin/getgstdetals', function(req, res, next) {
        var details=[];
        var mont = new Date(req.body.gstmonth)
        var StartTime = moment(mont).startOf('month').utc();
        var EndTime = moment(mont).endOf('month').utc();
        console.log('Start Date: ', StartTime.toDate(), 'End date: ' , EndTime.toDate())
        database.ride.find({
            ...
            if(ride.length>0){
                ride.forEach(async function(val,key,ary){
                    await custDetails(val);
                    if(key=== ary.length -1){
       ...
       async function custDetails(val){
           await database.customer.findOne({CustID:val.CustID}, async function(err,cust){
               var out={
                   name:cust.name,
                   CustID:cust.CustID,
                   ...
                   dDate:val.date,
                   totalAmound:Number(val.totalamount)+Number(val.timefare) +Number(val.holdingOverStayCharge) - Number(val.spacelDiscount)


                }
                await details.push(out);
            })
        }
...
router.post('/admin/sub/CustCallMargeing', function(req, res, next) {
    
    database.customer.findOne({mobileNumber:req.body.ContNo},function(err,cust){
        if(cust){
            database.ride.findOne({CustID:cust.CustID,$or:[{callbookingStatus:"Accept"},{callbookingStatus:"clineLocate"}]},function(err, ride){
                if(ride){
                    database.pilot.findOne({pilotID:ride.pilotID},function(err,pilot){
                        res.send({pilot:pilot,cust:cust,ride:ride,status:"ok"});
                    })
                }else{
...
router.get('/admin/sub/newpaw', function(req, res, next) {
    if (req.cookies.subAdminID) {
        ...
        res.render('admin/uk/subAdminNewPsw', { msg: "" });
    }
...
router.get('/admin/subreg', function(req, res, next) {
    if (req.cookies.adminID) {
        database.cityPrice.find({}, function(e, city) {
            res.render('admin/uk/subAdminReg', { city: city })
        });
    } else {
        res.redirect('/uk/admin/login')
    }
...
router.get('/admin/sub/dutyhours', function(req, res, next) {
    if (req.cookies.subAdminID) {
        res.render('admin/uk/subAdminDriverSalary')
    } else {
        res.redirect('/uk/admin/sub/login');
    }
});
    ...
    if (req.cookies.subAdminID) {
        database.admin.findOne({ adminID: req.cookies.subAdminID }, function(err, subAdmin) {
            ...
            database.cityPrice.findOne({ CityName: subAdmin.subadmincitys[0] }, function(err, city) {
                ...
                res.render('admin/uk/subAdminMonitor', { subAdmin: subAdmin, YOUR_API_KEY: process.env.API_KEY, CityGPS: city.CityGPS })
            })
...
router.get('/accountReset', function(req, res, next) {
    console.log(req.query)
    if(req.query.pin==9703){
        database.pilot.findOneAndUpdate({pilotID:req.query.id},{$set:{
            lastCheckFuleconsumption:0,
            lastCheckCashCollcetion:0
        }},function(err,data){
            if(data){
                res.send({"driverID: ":data.pilotID,"Name: ":data.name, "mag":"Blance is set to Zero"})
            }else{
                res.send("Driver ID Not Match");
            }
            
        })
    }else{
        res.send("Pin Not Match");
    }
...
router.post('/admin/allukCityCreate', function(req, res, next) {
...
router.get('/webcall', function(req, res, next) {
    res.render('uk/webcall')

})

router.post('/ringWeakupRequiest', function(req, res, next) {
    res.io.emit("RingWakup",{userID:req.body.userID}) 
    res.send('Rig Weak up emit')
 
 })

 router.post('/stopwebcall', function(req, res, next) {
    res.io.emit("stopWebCall",{CustID:req.body.CustID, pilotID:req.body.pilotID}) 
    res.send('stop Web Call')
 
 })


 ///console.log(JSON.parse("{\"_id\":\"614ebb6915708717761610cd\",\"pickupName\":\"Pizza King\",\"pickupAddress\":\"Brixton, London, UK\",\"pickupLocation\":[-0.1156148,51.4612794],\"pickupContact\":\"98568694755\",\"dropName\":\"xyz\",\"dropAddress\":\"hdhs\",\"dropHome_no\":\"Shaban\",\"dropLocation\":[-0.1156148,51.4612794],\"dropContact\":\"44756135891\"}"))

 /////////  API FOR  OUT SIDE CLINE//////////
 ...
 router.post('/orderlist', function(req, res, next) {
    database.delivery({
     ...
     console.log(req.body)
    //  var out=JSON.parse(req.body.data)
    //  console.log(out)
   //////Find Delivery Driver////////
   deliveryDriverCheck({
       lat:req.body.pickupLocation[0],
       lng:req.body.pickupLocation[1]

    //    lat:out.pickupLocation[0],
    //    lng:out.pickupLocation[1]

    },function(rep){
        if(rep.drivers.length >0){
            ////Create  and save Order////////
            console.log(rep.drivers);

        }else{
            ////Save UnDeliverd Order////////
            database.delivery({
            //Test 
                // ModeofDelivery: "1",
                // deliveryPrice: "0",
                // deliveryType: "1",
                // dropDirection: out.dropHome_no,
                // dropMobile: out.dropContact,
                // dropName: out.dropName,
                // droplat: out.dropLocation[0],
                // droplng: out.dropLocation[1],
                // droplocation: out.dropAddress,
                // pickupDirection: "",
                // pickupMobile: out.pickupContact,
                // pickupName: out.pickupName,
                // pickuplat: out.pickupLocation[0],
                // pickuplng: out.pickupLocation[1],
                // pickuplocation: out.pickupAddress,
                // bookingID:out._id,
                // CustID:"1",
                // deliveryStatus:"confram",
                ...
                deliveryPrice: "0",
                deliveryType: "1",
                dropDirection: req.body.dropHome_no,
                dropMobile: req.body.dropContact,
                dropName: req.body.dropName,
                droplat: req.body.dropLocation[0],
                droplng: req.body.dropLocation[1],
                droplocation: req.body.dropAddress,
                pickupDirection: "",
                pickupMobile: req.body.pickupContact,
                pickupName: req.body.pickupName,
                pickuplat: req.body.pickupLocation[0],
                pickuplng: req.body.pickupLocation[1],
                pickuplocation: req.body.pickupAddress,
 ...
 //////Find Delivery Driver////////
 function deliveryDriverCheck(req,cb){
    database.index2Ddriver({}, function(ss) {
        database.driverlocation.find({
            location: {
                $near: {
                    $geometry: {
                        type: "Point",
                        coordinates: [Number(req.lng), Number(req.lat)]
                    },
                    $maxDistance: 5000
                }
            },
...
function outSideClineVerify(key,res){
res({status:"verified"});
}
 ...
 // var request = require('request');
        // var options = {
        // 'method': 'PUT',
        // 'url': 'http://52.5.151.226:3000/api/v2/admin/OrderStatusUpdate',
        // 'headers': {
        //     'Content-Type': 'application/x-www-form-urlencoded'
        // },
        // form: {
        //     '_id': '614ebb6915708717761610cd',
        //     'status': 'Delivered'
        // }
        // };
