...
function addWallermoneyInit(){
    ...
    if (pass == "9703") {
        ...
        if(mobile!= null){
            $.post('/india/admin/getCusbalacce',{mobile:mobile},function(data){
                if(data!="Error"){
                    var money = prompt("Customer Name: "+data.name+" Wellet Balance :  "+data.wellet+"\n Enter Recharge Amount");
                    if(Number(money) > 0){
                        $.post('/india/admin/addWalletMony',{money:money,mobileNumber:data.mobileNumber,walletBalance:data.wellet},function(success){
                           alert(success)
                        })
                    }else{
    ...
    if (pass == "9703") {
        var bookingID = prompt("Enter Booking-ID");
            if(bookingID.length > 0){
                $.post('/india/admin/orderAmountModify',{bookingID:bookingID},function(data){
                    if(data!="Error"){
                        ...
                        if(Number(timefare)>0 && Number(prndingAmount)>0 ){
                            $.post('/india/admin/orderAmountUpdate',{
                                bookingID:bookingID,
                                timefare:timefare,
