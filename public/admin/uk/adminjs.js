...
function creatallukCityInit(){
    $("#cityPriceModule").html('<div class="col-md-10  col-md-offset-1 ">\
    <div class="panel panel-success">\
          <div class="panel-heading">\
                <h3 class="panel-title">All uk City <span id="totalof"> [ 0/0 ]</span></h3>\
          </div>\
          <div id="cityPriceList" class="panel-body">\
            <p class="text-center"><button onclick="creatallukCity(\''+1+'\', \''+1+'\')" type="button" class="btn btn-primary">Start</button></p>\
...
function creatallukCity(page,limit){
    if (confirm('Are You sure Careate All uk City It is effect Google Place API Bill')) {
        // Save it!
        $("#cityPriceList").html("Please Wait.....")
        $.post('/uk/admin/allukCityCreate',{page:page,limit:limit},function(data){
           if(data){
               if(data.nextPage){
                var nextPage='<li onclick="creatallukCity(\''+data.nextPage+'\', \''+data.limit+'\')" class="next"><a href="#">Next</a></li>'
               }else{
                var nextPage='<li class="next disabled"><a href="#">Next</a></li>'
               }
               if(data.prvPage){
                 var prvPage='<li onclick="creatallukCity(\''+data.prvPage+'\', \''+data.limit+'\')" class="previous"><a href="#">Previous</a></li>'  
                }else{
                 var prvPage='<li onclick="creatallukCity(\''+data.prvPage+'\', \''+data.limit+'\')" class="previous disabled"><a href="#">Previous</a></li>'  
                }
...
function creatNewBranch(){
    ...
    if(branchName){
        $.post('/uk/admin/addNewBranch',{branchName:branchName},function(data){
           if(data=="ok"){
            creatNewCityInit();
           }else{
               alert("Branch Exist")
...
function branchList(cb){
    $.post('/uk/admin/branchList',{},function(data){
        cb(data);
    })
}
...
function creatNewCity(){
    ...
    if(cityLat && cityLng && branch){
        $.post('/uk/admin/createNewCity',{cityLat:cityLat,cityLng:cityLng,branch:branch},function(city){
            //alert(city)
...
function getCityPriceList(){
    ...
    $.post('/uk/admin/getCityprice',{CityName:cityAry[0]},function(data){
       console.log(data)
       var out="";
...
function deleteCityPrice(id){
    ...
    $.post('/uk/admin/deleteCityPrice',{id:id},function(data){
        getCityPriceList();
    })
}
...
function incentiveCategory(){
   ...
   // alert(incetiveCity);
    $.post('/uk/admin/incentiveModuleAdmin',{incetiveCity:incetiveCity},function(data){
        if(data.length >0){
            var out="";
            data.forEach(function(val,key,ary){
...
function deleteIncentiveModule(enrtyID){
    $.post('/uk/admin/deleteIncentiveModule',{enrtyID:enrtyID},function(data){
        incentiveCategory();
    })
...
function addIncentiveModule(){
 ...
 // alert(incentiveTravelmod);
    $.post('/uk/admin/addIncentiveModule',{
...
function addWallermoneyInit(){
    var pass = prompt("Enter Authentication Number");
    if (pass == "9703") {
        var mobile = prompt("Enter Customer Mobile Number");
        if(mobile!= null){
            $.post('/uk/admin/getCusbalacce',{mobile:mobile},function(data){
                if(data!="Error"){
                    var money = prompt("Customer Name: "+data.name+" Wellet Balance :  "+data.wellet+"\n Enter Recharge Amount");
                    if(Number(money) > 0){
                        $.post('/uk/admin/addWalletMony',{money:money,mobileNumber:data.mobileNumber,walletBalance:data.wellet},function(success){
                           alert(success)
                        })
                    }else{
...
function orderAmountModify(){
    var pass = prompt("Enter Authentication Number");
    if (pass == "9703") {
        var bookingID = prompt("Enter Booking-ID");
            if(bookingID.length > 0){
                $.post('/uk/admin/orderAmountModify',{bookingID:bookingID},function(data){
                    if(data!="Error"){
                        var amount=Number(data.totalamount) + Number(data.timefare)-Number(data.spacelDiscount);
                        var timefare = prompt("Cust-ID: "+data.CustID+" Driver ID: "+data.pilotID+" \nAmount: "+amount+"\nEnter Time Fare:");
                        var prndingAmount=prompt("Enter Remening Amount:");
                        if(Number(timefare)>0 && Number(prndingAmount)>0 ){
                            $.post('/uk/admin/orderAmountUpdate',{
                                bookingID:bookingID,
                                timefare:timefare,
                                prndingAmount:prndingAmount,
                                paymentBy:data.paymentBy,
                                spacelDiscount:data.spacelDiscount,
