...
function getActiveDriverList(){
    ...
    $.post('/uk/admin/sub/monitorDriverList',{driverCity:driverCity,driveType:"preRide"},function(data){
     alert(data.length)
     console.log(data);
     if(data.length > 0){
      data.forEach(function(val,indx,ary){
        ...
        <span  style="font-size: medium;" class="badge"><div id="'+val.pilot.pilotID+'_currentlocation"><img onclick="mapp(\'' + val.driver.location.coordinates[1] + '\',\'' + val.driver.location.coordinates[0] + '\')" style="width: 20px; width: 10px; cursor: pointer;" src="/uk/images/maplogo.png"></div></span>\
        <span style="font-size: medium;" class="badge"><img style="width: 20px; width: 20px;" src="/uk/images/DriverMarker'+val.pilot.travelmod+'.png"></span>\
          '+val.pilot.name+'\
        </li>\
        <li class=" list-group-item list-group-item-info ">\
            <span style="font-size:small; color: chartreuse;" class="badge"><div id="'+val.pilot.pilotID+'_track"><img style="width: 22px;" src="/uk/images/gif/circle.gif"></div></span>\
    ...
    $.post('/uk/admin/sub/driverNFtotal',{},function(data){
      if(data){
        if(data.length > 0){
...
function getcurrentlocation(pilotID){
  $.post('/uk/admin/sub/monitorgetcurrentlocation',{pilotID:pilotID},function(data){
  ...
  function getcurrentbooking(pilotID){
    //alert(pilotID);
    $.post('/uk/admin/sub/monitorgetcurrentbooking',{pilotID:pilotID},function(data){
      //console.log(data);
...
function trackdriver(listDrv){
  ...
  listDrv.forEach(function(val,indx,aryt){
    ...
    $("#"+val+"_track").html('<img style="width: 22px;" src="/uk/images/gif/circle.gif">')
    console.log("test Dtiver ID", val)
    $.post('/uk/admin/sub/senddrivertracrequiest', {pilotID:val}, function(data){      
      if(data){
        cot++;
        console.log(data);
        if(indx===aryt.length -1 && cot==aryt.length){
          ...
          setTimeout(function(){
            ...
            if(exist < 0){
              //////Tracking Fail////
              console.log("Fail", dd)
              $("#"+dd+"_track").html('<img style="width: 22px;" src="/uk/images/trackingfail.png">')
            }else{
              /////Track Success////// 
              console.log("success", dd)           
              $("#"+dd+"_track").html('<img style="width: 22px;" src="/uk/images/tracking.png">')  
            }
            })
          },500);
        }
...
function gotodemand(subAdminID){
  ...
  $("#gotodemand").html('<img style="width: 40px;" src="/uk/images/gif/progress.gif">');
  demandtimer=setInterval(function(){
    ///////call demand area////////
    console.log("subAdminID",subAdminID)
    
    
    $.post('/uk/admin/sub/gotodemandrequiest', {lat:demandlatlng[0],lng:demandlatlng[1],pilotID:demandDriverid,subAdminID:subAdminID}, function(data){ 
...
function getDriverNotFound(){
  ...
  $.post('/uk/admin/sub/gatherDriverNF',{},function(data){
    if(data.length>0){
        var out="";
...
function findCust(entryNo,CustID){
  ...
  $.post('/uk/admin/sub/driverNFcust',{CustID:CustID},function(data){
    if(data){     
      $("#cust"+entryNo+"").css({"display":"block"});
      $("#cust"+entryNo+"").html('Cust Name: '+data.name+'<br>\
      Mobile No: +44 '+data.mobileNumber+'\
      Email Id : '+data.email+'')
    }
  })
...
function resolve(entryNo,CustID){
  $.post('/uk/admin/sub/driverNFresolve',{entryNo:entryNo},function(data){
    if(data){
...
function calculateOrder(){
  ...
  if(mPickuplatlng && mDroplatlng && mCustMobile && mDriverID && travelTypemode){
    ...
    $.post('/uk/getDistance',{
      ...
      $.post('/uk/subAdminGetprice',{travelmod:travelTypemode,distance:alldist,time:time,hour:hour,mm:mm,custMobile:mCustMobile},function(dataa){
        if(dataa.errormsg){
...
function onbehalfCusOrder(){
  ...
  $.post('/uk/admin/sub/PreRideCallAndBooking',{
...
function deliveryDetails(){
  ...
  $.post('/uk/admin/sub/getDeliveryList',{},function(data){
    console.log(data);
