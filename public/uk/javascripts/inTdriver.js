  ...
  var ln=JSON.parse(getCookie("reglatlng")) 
    ...
    function wachLocation(){ 
      wachID= setInterval(function(){
        $.post('/uk/GetTdriverLocation',{},function(driverloc){
          if(driverloc){
...
/////OFFline  /////////
 function offlineExecute(){
    ...
    $.post('/uk/TrideDutyoffline',{},function(data){ 
      console.log(data);
    })
 ...
 $.post('/uk/tdrv/CityFulepriceUpdate',{pilotID:pilotID},function(cityFule){        
   if(cityFule=="0"){
 ...
 function pageInitiate(){
  ...
  if($("#bookingID").val()){
    $.post('/uk/tdrv/getPageInitiateDetails',{bookingID:$("#bookingID").val()},function(data){ 
      ...
      if($("#orderStage").val()=='accept'){
       ...
       $("#telsms").html('<a onclick="callinit(\'' + data.ride.CustID+ '\')"><button type="button" class="btn btn-warning btn-ms"><i class="fa fa-phone" aria-hidden="true"></i></button></a>\
                    <a ><button type="button" class="btn btn-warning btn-ms"><i class="fa fa-comments" aria-hidden="true"></i></button></a>');
...
function incetiveAndBooking(){
  //alert("ddd")
  $.post('/uk/tdrv/bookingIncentiveDetails',{},function(data){
...
$.post('/uk/tdrv/confrmAcceptWindowOpenProperly',{pilotID:data.pilotID},function(dat){
  ...
  setTimeout(function(){
      $("#ringtone").css({"display":"none"});
      $.post('/uk/tdrv/resetRingtone',{pilotID:data.pilotID},function(dat){
          console.log(dat);
      });
  },14*1000)
     })
    ...
    function acceptRide(){                        
      ...
      $.post('/uk/tdrv/AcceptCallByTdriver',{pilotID:pilotID,CustID:CustID},function(data){            
      if(data){ 
          $("#ringtone").css({"display":"none"});
        ...
        socket.on('CallAcceptListDisplay', function (resp) {            
          ...
          if(resp.pilotID==getCookie("pilotID")){
              $.post('/uk/tdrv/clineDetalls',{pilotID:resp.pilotID,CustID:resp.CustID,bookingID:resp.bookingID},function(data){
                  $("#pickDrop-Content").css({"display":"block"});
                  $("#orderNO").text(data.ride.bookingID);
                  $("#bookingID").val(data.ride.bookingID)
                  $("#CustID").val(data.ride.CustID);
  ...
  if(a==1){
      ...
      ((navigator.platform.indexOf("iPhone") != -1) || 
       ...
       (navigator.platform.indexOf("iPod") != -1)){
        ...
        $.post("/uk/tdrv/startCarLoogbook",{bookingID:bookingID},function(data){console.log(data)});
        $("#clineLocated").css({"display":"block"});
...
function clineLocated(){ 
  $.post('/uk/tdrv/clinelocatedCheckDistance',{CustID:$("#CustID").val(),bookingID:$("#bookingID").val()},function(respon){
...
function clineLocatedStart(CustID, bookingID){
  ...
  $.post("/uk/tdrv/CarLoogbookClineLocate",{CustID:CustID,bookingID:bookingID},function(data){
    console.log(data)
    if(data=="error"){
...
if(valu.length >3){
  ...
  if($("#OrderOTP").val()==valu){
      $.post('/uk/tdrv/startRide',{CustID:$("#CustID").val(),bookingID: $("#orderNO").text()},function(data){
...
function finishride(){   
  ...
  $.post('/uk/tdrv/finishCheckDistance',{CustID:$("#CustID").val(),bookingID:$("#bookingID").val()},function(respon){
    console.log("respon",respon)
...
function finishRideStart(CustID,bookingID){
  $.post('/uk/tdrv/finishRide',{CustID:CustID,bookingID:bookingID},function(respon){
    if(respon){           
                $("#pickDrop-Content").css({"display":"none"});
                $("#billAndfeedback").css({"display":"block"});                  
                $("#OTP-Content").css({"display":"none"});
...
function ledger(){
  ...
  $.post('/uk/tdrv/ledger',{},function(data){
    ...
    if(Number(data.driverCetegory)== 1){
      ...
      if(Number(data.previousDue)>=Number(data.previousEarning)){
        ...
        btnn='<form name="myForm" action="/uk/withdrawal" onsubmit="return validateForm()" method="POST" class="form-inline" role="form">\
...
function driverPaymentBreakdown(){
  ...
  $.post('/uk/tdrv/paymentBreakdown',{distance:i},function(data){
...
function incentive(){
  ...
  //alert(incentiveTravelMode)
    $.post('/uk/admin/incentiveModule',{incetiveCity:incetiveCity,incentiveTravelMode:incentiveTravelMode},function(data){
        if(data.length >0){
  ...
  <div  class="panel-body">\
      <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">\
          <a   href="#" class="thumbnail">\
              <img src="/uk/images/banner-1.png" alt="">\
          </a>\
