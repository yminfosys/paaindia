    ...
    allmarkerList.forEach(function(val,indx){
     // console.log("markerValue",val)
       driverMarkers.push(new google.maps.Marker({
         ...
         icon:{
             url: "/uk/images/DriverMarker"+val.tm+".png", // url
...
directionsService.route(reqst, function(response, status) {
  ...
  document.getElementById("naxtBtn").addEventListener("click", function(){ 
    ...
    directionRooteService(origin,dist,2,function(resp){
     ...
     var alldist=parseInt(Number(resp.distance)*0.00062137) + 1; 
     var time=Number(resp.time)/60;
      ...
      $.each($(".modeImg"),function(i){
        ...
        $.post('/uk/getPrice',{travelmod:j,distance:alldist,time:time,hour:hour,mm:mm},function(dataa){
...
$("#promoMsg").appendTo(map.getDiv());
  ...
  if(iOS()){
    //////Back Btn////
    document.getElementById("iosBaceBtn").addEventListener("click", function(){ 
      window.history.back();
    })
   $('<div/>').addClass('backiosBtn').appendTo(map.getDiv())
   .click(function() { 
    window.history.back();
   })
