...
function registerprocess(){
  ...
  $.post('/uk/newcustreg',{name:name,email:email,password:password,mobile:mobile,reffrom:reffrom,refBy:refBy},function(data){
    if(data){
...
////////CAll pickup placesearch content//////
    ...
    function pickupsearch(a){
      ...
      var out='<a onclick="CloseAll(\'placesearch\')" class="list-group-item active"><i class="fa fa-map-marker" aria-hidden="true"></i></span> &nbsp; Select From Map</a>';
        $.post('/uk/previousplacesearch',{type:a,CustID:$("#CustID").val()},function(data){
  ...
  //alert(CustID);
      ...
      $.post('/uk/rideDetails',{CustID:CustID},function(data){
        console.log(data)
        if(data.length > 0){
    ...
    function ScheduleJourney(){
      ...
      CustID=$("#CustID").val();
          ...
          $.post('/uk/getScheduleJourney',{CustID:CustID},function(data){
            console.log(data);
            if(data.length > 0){
              var out='<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">\
            <ul style="height: 91vh; overflow-y: auto; margin-top:50px; " class="list-group">\
    ...
    function deleteSchdule(bookingID){
      $.post('/uk/deleteSchdule',{bookingID:bookingID},function(data){
        if(data=="OK"){
          ScheduleJourney();
          sidebartoggle();
        }
          ...
          <div style="margin-top: 5px;" class="panel panel-primary">\
              ...
              <div style="height: 60vh; overflow-y: auto;" class="panel-body">\
                  <div  style="float: left; width: 90%; border-radius: 5px;  background-color: rgb(238, 230, 221); ">\
                    <a style="float: left; padding: 3px; color: #000;"><img width="20px" src="/uk/images/gif/progress.gif"> Chat is Connecting ...  </a>\
                </div>\
              </div>\
          </div>\
  ...
  countingWatch('time',1);
        ...
        if(data.CustID==CustID){
          ...
          <div class="row">\
            <div class="col-xs-12 col-sm-12">\
              <div style="margin-top: 5px;" class="panel panel-primary">\
                    <div class="panel-heading">\
                          <h3  class="panel-title"><img width="30px" src="/uk/images/custLogo.png"> Paa Admin\
    ...
    function sendChatContent(chatSession){
      ...
      $.post('/uk/sendChatContent',{chatSession:chatSession,chatContent:chatContent,userID:CustID},function(data){
        if(data){
    ...
    function chatisConnectingRequest(CustID){
      $.post('/uk/chatisConnecting',{CustID:CustID},function(data){
        if(data){
    ...
    function icicipay(){
      $("#waletpay").html('<img style="width: 40px;" src="/uk/images/gif/progress.gif"><br><label>** Do not Refresh or back during paymet process ** </label>')
      ...
      Layer.checkout({
        ...
        function(response) {
          ...
          $.post('/uk/icici/paydetails',{payment_token_id:response.payment_token_id,status:response.status},function(data){
            ...
            if(data){
              window.location.href='../uk?wallet="success"';
  ...
  //         window.location.href='/uk/?geocode=1'
   ...
   function continueBooking(payMode,type){
    ...
    $.post('/uk/nearbyTrideBooking',{
      ...
      if(data.drivers.length > 0){
         ...
         /////Call to Driver///
         var count=0; 
         $.post('/uk/CallTdriver',{pilotID:data.drivers[count].pilotID,CustID:CustID,pickuoAddress:originAds},function(result){
            console.log(result);
          }); 
         count++;
         timere=setInterval(function(){ 
         ...
         socket.on('TdriverAccepeCall', function (incomdata) {
          if(CustID==incomdata.CustID){
            ...
            tttt=setTimeout(function(){
              ///////Grnerate Order/////
              $.post('/uk/savetdriverCallAndBooking',{
                originAds:originAds,
                distAds:distAds,
                originLat:origin.lat,
                originLng:origin.lng,
    ...
    var pickuplat=$("#pickuplat").val();
        ...
        $.post('/uk/setpickuptimesetup',{
          ...
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">\
              ...
              <p><img style="width: 100%; height: 100px;" src="/uk/images/thank.png"></p>\
          </div>\
          <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">\
