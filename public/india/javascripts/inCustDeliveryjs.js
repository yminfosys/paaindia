 ...
 function ContinueNext(changeType){
  ...
  var deliveryPrice=$("#deliveryPrice").val();
    ...
    <div class="col-xs-4 col-sm-4 col-xs-offset-4 col-sm-offset-4">\
        <button type="button" id="PriceBtn" class="btn btn-primary col-xs-12 col-sm-12">&#8377;  '+Number(deliveryPrice).toFixed(2)+'</button>\
    </div>\
    <div class="col-xs-8 col-sm-8 col-xs-offset-2 col-sm-offset-2">\
    <input onkeyup="modifyPrice()" type="text" style="margin-top: 5px;" class="form-control" id="compensetion" placeholder="Entet Compensetion Amount">\
    </div>\
...
console.log(inp)
 ...
 function modifyPrice(){
  clearTimeout(timeoin);
  timeoin=setTimeout(function(){
  var deliveryPrice=$("#deliveryPrice").val();
  var compensetion=$("#compensetion").val();
  var newPrice=Number(deliveryPrice)+(Number(compensetion)*0.005)
  $("#PriceBtn").html('&#8377;'+newPrice+'');
